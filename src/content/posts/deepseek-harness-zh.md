...
> DeepSeek 开源了一个 Agent Harness。我花了两天时间翻完仓库，40+ 包、20+ 个能力接缝，最值得注意的是——Agent Loop 本身是个插件，可以从 YAML 配置里换掉。

![一切皆插件示意](/images/deepseek-harness/deepseek-harness-everything-plugin.png)

## 先说结论

翻完仓库我最大的感受是：这个团队对"插件化"有一种追求。

不是那种"我们支持插件扩展"的程度，而是**整个产品——包括 Agent Loop——都是插件拼出来的**。没有一个"特权核心"是你必须接受它的实现的。想换 LLM 适配器？写一个插件。想换文件系统后端？写一个插件。想换 Agent Loop 本身？写一个插件。

跟 Pi 刚好是两个极端。Pi 做减法到极致，DeepSeek Harness 做可替换性到极致。都有优缺点，看你的场景。

先看三个最关键的差异：

**Agent Loop**：Pi 写死，Claude Code 写死，DeepSeek Harness 可替换——你可以从 YAML 配置里换掉整个执行循环。

**插件化程度**：Pi 是轻扩展，DeepSeek Harness 是一切皆服务，连 Agent Loop 本身都是插件拼出来的。

**自修改能力**：其他框架是人去配置它，DeepSeek Harness 的 Agent 可以自己挂载/卸载插件。

这个"一切皆插件"的设计，代价是复杂度——40+ 个包的学习曲线不低。但收益也很明显：任何一层实现不满意，都可以换掉。
...
```sh
# 一行启动 Web UI，默认 http://127.0.0.1:3080
npx @deepseek-ai/dsh web
```

40+ 个包。不是"核心 + 几个插件"那种结构，是**整个产品都被拆成了模块**。下面挑几个我觉得最值得看的设计。

## Agent Loop 是插件

这个最让我意外。Agent Loop 是所有框架里最"核心"的部分，在 DeepSeek Harness 里居然可以被替换。

大多数 Agent 框架的 Loop 是写死的——你想改执行策略就得改核心代码。DeepSeek Harness 把 Loop 做成了一个 `ctx.agentLoop` 服务，跟其他服务平级。你可以在 `cordis.yml` 里指定用哪个实现。

整个产品基于一个叫 Cordis 的插件框架（有篇论文叫 [_A Programming Paradigm for Spatiotemporal Composability_](https://github.com/cordiverse/paper)），核心概念不多：
...
里面最值得说的是 **Effects 的可逆性**。插件注册的东西（工具、prompt 片段、事件监听器）都走 `ctx.effect()` 或 `ctx.on()`，卸载时自动清理。听起来简单，但我翻过不少 Agent 框架的插件系统，真正做到的几乎没有——要么注册了就一直在，要么卸载了但状态没清干净。Cordis 底层提供了 effect 机制，DeepSeek Harness 是我见过的第一个在 Agent 框架层面把它用到位的：注册即副作用，卸载即清理，做成了框架级保证。
...
## Capability Seam：三层角色分离

"加一个能力"在大部分框架里就是写个函数注册到工具表。出了问题翻源码找接口，想换后端得改 Consumer。
...
换 Provider 不用动 Consumer。想从本地执行切到 E2B 远程沙箱，换一个 provider 包就行，工具定义完全不需要改动。
...
子代理有 6 种后端值得关注，包括直接对接 Codex 和 Claude Code 的 provider。沙箱也做了三种（bwrap / Landlock / Seatbelt），分别对应 Linux / macOS 新版安全模块 / macOS 传统沙箱。

## Session Log：模型看到的东西必须在日志里
...
Pi 走的是"无状态"路线，DeepSeek Harness 走的是"一切有状态，状态全在日志里"。两种都能 work，但我个人更倾向 DeepSeek 的做法——**"model-visible ⟺ logged"这条断言，是我见过最有传播价值的一个 Agent 设计原则**。它把"模型看到的东西必须可追溯"从一条最佳实践变成了一个运行时检查。

## Profile + Bundle + Patch：配置即组合
...
两个方向：Pi 是"能砍就砍"，DeepSeek Harness 是"能拆就拆"。共同点是想清楚了"什么该在框架里，什么不该在"——Pi 的答案是"尽量少"，DeepSeek 的答案是"全部可插拔"。

选哪条路看场景。但如果你在做 Agent 平台，Capability Seam 三层分离、Session Log 事实源、Agent Loop 可替换——这三个设计思路都值得深入考虑。

---

## 参考资料

- [DeepSeek Harness 仓库](https://github.com/deepseek-ai/deepseek-harness) — README、AGENTS.md、architecture.md、development.md、cordis-primer.md、packages/README.md 已核对
- [Cordis 框架](https://github.com/cordiverse/cordis)
- [Cordis 论文：A Programming Paradigm for Spatiotemporal Composability](https://github.com/cordiverse/paper)（未逐字核对）
- 对照参考：[Pi Agent 设计哲学剖析](/posts/pi-agent-design-philosophy)
