...
- 验证有效的经验是升格到 Rules / Knowledge 的原料；同时也允许降级，详见 §五。
...
> 有意思的是，[Anthropic 官方](https://www.anthropic.com/engineering/building-effective-agents) 和我们这套落地天然呼应：Agent 应该周期性地把目标、进度、关键决策、未解决问题写到上下文之外的笔记文件里，上下文清空后读回。**记忆是"精挑细选的工作状态"，不是对话的转录垃圾。**
...
1. **小项目 / 个人项目**：一个 `CLAUDE.md` 可能已经是全部。六个规划目录建好却全是空的，那不是记忆管理，是仪式感。
...
*这个框架来自在生产环境中使用 AI 辅助开发一年多的经验。它从简单的 `CLAUDE.md` 方案演进为一个结构化系统，可以跨项目和团队规模扩展。*